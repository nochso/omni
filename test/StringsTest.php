...
class StringsTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPadMultibyte($expected, $input, $padLength, $padding, $paddingType, $message = '') {
		$this->assertSame(
			$expected,
			Strings::padMultibyte($input, $padLength, $padding, $paddingType),
			$message
		);
	}
	...
	public function padMultibyteProvider() {
		return [
			...
			[
				...
				'x',
				STR_PAD_BOTH,
				'Return original already long enough',
			],
		];
	}
	...
	public function groupByCommonPrefixProvider() {
		return [
			[['/root/' => ['bar/baz', 'foo']], ['/root/foo', '/root/bar/baz']],
			[['' => ['/root/bar/baz', 'x/root/foo']], ['x/root/foo', '/root/bar/baz']],
			[['/root' => ['', '/foo']], ['/root/foo', '/root']],
		];
	}
